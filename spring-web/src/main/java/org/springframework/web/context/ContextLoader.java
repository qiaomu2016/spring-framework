...
public class ContextLoader {
	 ...
	 *
	 *
	 * [1] 处，配置了 org.springframework.web.context.ContextLoaderListener 对象。
	 * 		这是一个 javax.servlet.ServletContextListener 对象，会初始化一个Root Spring WebApplicationContext 容器。
	 * [2] 处，配置了 org.springframework.web.servlet.DispatcherServlet 对象。
	 * 		这是一个 javax.servlet.http.HttpServlet 对象，它除了拦截我们制定的 *.do 请求外，也会初始化一个属于它的 Spring WebApplicationContext 容器。
	 * 		并且，这个容器是以 [1] 处的 Root 容器作为父容器
	 *
	 * 为什么有了 [2] 创建了容器，还需要 [1] 创建了容器呢？因为可以配置多个 [2] 呀。当然，实际场景下，不太会配置多个 [2] 。😈
	...
	static {
		...
		// 这是一个应用开发者无需关心的配置，而是 Spring 框架自身所定义的
		...
		// 如果我们没有在 <context-param /> 标签中，配置指定的 WebApplicationContext 类型，就使用 XmlWebApplicationContext 类。一般情况下，我们也不会主动指定。
		try {
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		// 获得 context 的类：default XmlWebApplicationContext or a custom context class if specified
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		// 刷新 context ，执行初始化  （org.springframework.context.support.AbstractApplicationContext#refresh）
		wac.refresh();
	}
